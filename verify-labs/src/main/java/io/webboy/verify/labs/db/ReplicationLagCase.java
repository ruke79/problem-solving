...
public class ReplicationLagCase extends VerificationCase {
    ...
    protected void verify(Evidence evidence) throws Exception {
        ...
        waitForReplica(3_000);   // DDL 이 레플리카에 반영될 때까지
        ...
        try (Connection replica = DriverManager.getConnection(replicaUrl, username, password)) {
            ...
            boolean caughtUp = waitUntilReplayed(replica, writeLsn, 5_000);
            ...
            evidence.expectFlaky("쓰기 직후 즉시 읽으면 아직 안 보일 수 있다(read-your-writes 위반)",
                    immediately == 0);
        } finally {
            primary.execute("DROP TABLE IF EXISTS replica_demo");
        }
    ...
    private void waitForReplica(long millis) throws InterruptedException {
        Thread.sleep(millis);
    }
